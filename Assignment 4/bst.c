...
{
	/*write this*/
    struct Node *newNode; // make new node
    assert(val != 0); //aeert that the val is not NULL
    //deciding where the new node will go
    // in case there is no root node
    if (cur == 0) { // if the current root node is equal to NULL, meaning there is no root node
        newNode = (struct Node *)malloc(sizeof(struct Node)); //make new node, this node will become root node
        assert(newNode != 0); // assert that node was created
        newNode->val = val; // assign value in the node
        newNode->left = newNode->right = 0;
        return newNode;
    }
    //in case there is a root node, we need to decide where the child will go RIGHT vs. LEFT
    if (compare(cur->val, val) <= 0){   //if cur->val <= val, the new node will go to the right
        cur->right = _addNode(cur->right, val); // adding new node to the right
    }
    else{  //finally if cur->val > val, new node will go to the left
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    assert(tree != 0);
    assert(val != 0);
    struct Node *current = tree->root; //set the current to the index of the root node
    while (current != 0){//loop through until reaches the end of the tree
        if (compare(val, current->val) == 0) { //if you find the val, return true
            return 1;
        }
        else if (compare(val, current->val) < 0){//if val < than the value in the current  node, go to the left
            current = current->left;
        }
        else{ //if the val > than the value in the current node, go to the right
...
{
	/*write this*/
	//loop through until leftmost child is NULL
	while(cur->left !=0)
        cur = cur->left;
    return cur->val;
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	/*write this*/
	struct Node *node;
	if (cur->left == 0){ //if at leftmost
	    node = cur->right; //set the current node to the right child to return it
	    free(cur); // free the current
	    return node;
	}
	cur->left = _removeLeftMost(cur->left); // recursively find the leftmost child
	return cur;
}
/*
...
{
    //WRITE THIS FUNCTION
	assert(cur != 0); //cur in not null
    assert(val != 0); // val is not null

    struct Node *temp; //create temp node
    if (compare(cur->val, val) == 0) { // if the value is found
        if (cur->right == 0){// if there is no right child
            temp = cur->left;  // store left child in tem node
            free(cur); // delete the node
            return temp; // return the left child
        }
        cur->val = _leftMost(cur->right);  //swap current node value with the leftmost of right child value
        cur->right = _removeLeftMost(cur->right); //set right child to removeleftmost
    }
    else if (compare(val, cur->val) == -1){ //if val < than cur->val
        cur->left = _removeNode(cur->left, val); //we are  go to the left recursively
    }
    else{ //if val >= than cur->val
        cur->right = _removeNode(cur->right, val);// we go to the right recursively
    }
    return cur;
}
/*
