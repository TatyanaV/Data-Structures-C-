...
void swapDynArr (struct DynArr * da, int i, int j) {
    ...
    DynArr temp;// do we really need a struct variable?  // what is the advantage of using struct is TYPE???
     //also lets add couple of assert function:
    assert(i <= da->size);
    assert(j <= da->size);
    assert(i >= 0);
    assert(j >= 0);
...
void removeAtDynArr (struct DynArr * da, int index) {
            ...
            int i;
            //lets add few assert functions
            assert(index < da->size);
            assert(index >= 0);

            // i thing this needs to be changed: for ( i = index; i < DynArr->size; i++)
            for(i = idx; i <= v->size-2; i++){

                       //this needs to be changed as well :DynArr->data[index] = DynArr->data[index+1];
                        da->data[index] = DynArr->data[index+1];
            //this needs to be changed too DynArr->size--;
            da->size--;
}
/* ************************************************************************
